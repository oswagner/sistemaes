...
prototipo.controller("contentController", [
	'$scope',
	'$http',
	...
	function($scope, $http) {
	    ...
	    $scope.enterpriseCalbackFunction = function(data) {
		    console.log("calback id"+data);
		    $http({
			    method : 'POST',
			    url : '/prototipo/service/enterprise/findEnterprise',
			    data : data,
			    headers : {
				'Content-Type' : 'application/json'
			    }
			}).success(function(data, status, headers, config) {
			    $scope.empresaLive= data;
		...
		$scope.empresaJson = angular.toJson(empresa, true);
		...
		empresa.cidade = '';
		empresa.email = '';
		empresa.qtdFuncionariosTi = null;
		empresa.qtdFuncionarios = null;
		empresa.isAgil = null;
		empresa.agilTempo = '';

		$http({
		    method : 'POST',
		    url : '/prototipo/service/enterprise/register',
		    data : $scope.empresaJson,
		    headers : {
			'Content-Type' : 'application/json'
		    }
		}).success(function(data, status, headers, config) {
		    console.log("return id"+data);
		    $scope.enterpriseCalbackFunction(data);
		    console.log("empresaID="+$scope.empresaID);
		}).error(function() {
		...
		$scope.searchProblem = value;
	    };
	    
	    
	    $scope.problemCalbackFunction = function(data) {
		$scope.problems = data;
	    };

	    $scope.getAllProblems = function() {
		$http.get('/prototipo/service/problems/findAllProblems')
			.success(function(data, status, headers, config) {
			    $scope.problemCalbackFunction(data);
			}).error(function(data, status, headers, config) {
			    console.log('erro na request');
			});
	    ...
	    $scope.addProblem = function(value, element) {
		
		var classes = element.currentTarget.className + ' btn-success';
		element.currentTarget.className = classes;
		console.log(element.currentTarget.className);
		$scope.selectedProblems.push(value);
		
		var ids = $scope.selectedProblems;
		var uniqueIds = [];
		$.each(ids, function(i, el){
		    if($.inArray(el, uniqueIds) === -1) uniqueIds.push(el);
		});
		$scope.selectedProblems = uniqueIds;
	    };
	    	    
	    $scope.removeProblem = function(value) {
		
		var array = $scope.selectedProblems;
		for (var i = 0; i < array.length; i++) {
		    if(array[i] === value){
			array.splice(i);
		    }
		}
		$scope.selectedProblems = array;
		...
		var data = angular.toJson(problems, true);
		
		$http({
		    method : 'POST',
		    url : '/prototipo/service/solutions/findSolutionsByProblems',	
		    data : data,
		    headers : {
			'Content-Type' : 'application/json'
		    }
		}).success(function(data, status, headers, config) {
		    $scope.solutionList = data;

		      var indexedSolutions = [];
		      
		      $scope.solutionsToFilter = function() {
			  indexedSolutions = [];
		          return $scope.solutionList;
		      };
		      
		      $scope.filterSolutions = function(solution) {
		          var solutionIsNew = indexedSolutions.indexOf(solution.nomeProblema) == -1;
		          if (solutionIsNew) {
		              indexedSolutions.push(solution.nomeProblema);
		          }
	    ...
	    $scope.cidades = [ "Aceguá", "Água Santa", "Agudo", "Ajuricaba",
